							...
							<table width="99%" border="0" cellpadding="1" cellspacing="1"
								...
								<c:forEach items="${odersList}" var="obj" varStatus="con">
									<tr>
										...
										<td bgcolor="#FFFFFF"><c:if test="${obj.flag == 0}">
                			没取车               		</c:if> <c:if test="${obj.flag== 1}">
                			已经取车                		</c:if> <c:if test="${obj.flag == 2}">
                			交易完成                		</c:if></td>
										<td bgcolor="#FFFFFF">&nbsp;&nbsp;&nbsp;<a
											href="odersupdate_qflag/${obj.id}">确认交易</a>
										</td>
									</tr>
								</c:forEach>
							</table>
						<!--  
							<p style="text-align: center">
								<jsp:include page="/pageinfo.jsp">
									<jsp:param value="orders_list" name="path" />
								</jsp:include>
							</p>
-->
